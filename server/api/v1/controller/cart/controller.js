...
async addProductToCart(req, res, next) {
        ...
        try {
            const { items } = await validationSchema.validateAsync(req.body);
            const { productId, inventoryId, quantity, colors, sizes } = items[0];
            ...
            let cart = await cartModel.findOne({ userId: req.userId, status: status.ACTIVE });
            if (!cart) {
                cart = new cartModel({ userId: req.userId, items: [], subtotal: 0, status: status.ACTIVE });
            }
            ...
            if (existingItemIndex !== -1) {
                ...
                existingItem.totalAmount = newQuantity * productPrice;
            } else {
            ...
            const addedItem =
                existingItemIndex !== -1 ? cart.items[existingItemIndex] : cart.items[cart.items.length - 1];
 ...
 */
 ...
 async updateProductToCart(req, res, next) {
        ...
        try {
            const { _id, itemId, quantity } = await validationSchema.validateAsync(req.body);
            ...
            const cart = await cartModel.findOne({ _id, userId: userData._id, status: status.ACTIVE });
